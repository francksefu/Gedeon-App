...
<?php
  function dataVentes(){
    ...
    $sql = ("SELECT * FROM Ventes order by idVentes desc");
    $result = mysqli_query($db, $sql);
            
    if(mysqli_num_rows($result)>0){
                        
        while($row= mysqli_fetch_assoc($result)){
            echo"<option value='ID ::".$row["idVentes"].":: PA ::".$row["PA"].":: PV  ::".$row["PV"].":: Benefice ::".$row["Benefice"].":: Dates ventes ::".$row["DatesVente"].":: Nom produit ::".$row["NomProduit"].":: Motif ::".$row["Commentaire"]."'>produit = ".$row["NomProduit"]." PV : ".$row["PV"]."</option>"; 
        }
...
<body class="bg-light">
    ...
    <main>
        <div class="container bg-transparent pt-5" >
            <div class="row bg-transparent pt-5">
                <div class="col-md-6 bg-transparent m-2">
                    <h2>Liste des vos ventes ou cop</h2>
                    <p class=" text-secondary pt-3">
                        Cette table repertories vos ventes ou cop, genre 
                        lorsque vous achetez quelque choses avec une intention de faire de benefices 
                    </p>
                </div>
            ...
            <div class="input-group mt-3 col-md-10 montre-moi">
                ...
                <input type="hidden" value="ventes" id="type" >
        </div>
        ...
        <?php
                ...
                $reqSql= ("SELECT * FROM Ventes order by idVentes desc");
